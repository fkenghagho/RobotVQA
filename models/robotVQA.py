...
def detection_targets_graph(Inputs, config):
    ...
    relations:    [MAX_GT_INSTANCES,MAX_GT_INSTANCES,4]
    ...
    relations: [TRAIN_ROIS_PER_IMAGE,TRAIN_ROIS_PER_IMAGE,4]
    ...
    roi_gt_class_ids[5] = tf.pad(roi_gt_class_ids[5], [(0, N + P),(0, N + P),(0, 0)])
...
class DetectionTargetLayer(KE.Layer):
    ...
    relations: [batch,MAX_GT_INSTANCES,MAX_GT_INSTANCES,4]
    ...
    relations: [batch,TRAIN_ROIS_PER_IMAGE,TRAIN_ROIS_PER_IMAGE,4]
    ...
    def compute_output_shape(self, input_shape):
        return [
            ...
            (None,self.config.TRAIN_ROIS_PER_IMAGE, self.config.TRAIN_ROIS_PER_IMAGE, 4),# relations
...
def refine_detections_graph(Inputs, config):
    ...
    Inputs:
        ...
        probs: [N, NUM_FEATURES, num_classes]. Class probabilities.
    ...
    Returns detections shaped: [N, (y1, x1, y2, x2, class_ids, scores,poses)] where
...
class DetectionLayer(KE.Layer):
    ...
    Returns:
    [batch, num_detections, (y1, x1, y2, x2, class_ids, class_score,poses)] where
    coordinates are in image domain
    """
    ...
    def call(self, inputs):
        ...
        _, _, window, _,_,_,_,_ = parse_image_meta_graph(image_meta,self.config)
        ...
        return tf.reshape(
            detections_batch,
            [self.config.BATCH_SIZE, self.config.DETECTION_MAX_INSTANCES, 4+self.config.NUM_FEATURES*2+6])
...
def fpn_classifier_graph(transform_func,rois, feature_maps,
    ...
    for i in range(1,5):
        relational_graph_input.append(transform_func(shared,i))
        print(relational_graph_input[i-1].shape)
    ...
    # Classifier head
    mrcnn_class_logits=[]
    mrcnn_class_logits.append(KL.TimeDistributed(KL.Dense(num_classes[0]),
                                            name='mrcnn_class_logits0')(shared))
    mrcnn_class_logits.append(KL.TimeDistributed(KL.Dense(num_classes[1]),
                                            name='mrcnn_class_logits1')(shared))
    mrcnn_class_logits.append(KL.TimeDistributed(KL.Dense(num_classes[2]),
                                            name='mrcnn_class_logits2')(shared))
    mrcnn_class_logits.append(KL.TimeDistributed(KL.Dense(num_classes[3]),
                                            name='mrcnn_class_logits3')(shared))
    ...
    relational_graph_reshape=KL.Reshape([int(nber_boxes),int(nber_boxes),int(num_classes[5])],name="rel_reshape0")
    relational_graph_logits=KL.TimeDistributed(KL.Dense(num_classes[5]),name='mrcnn_class_logits5')                                        
    for i in range(len(relational_graph_input)):
        y= relational_graph_logits(relational_graph_input[i])
        print('y',y.shape)
        relational_graph_output['logits'].append(relational_graph_reshape(y))
    ...
    mrcnn_probs.append(KL.TimeDistributed(KL.Activation("softmax"),
                                     name="mrcnn_class0")(mrcnn_class_logits[0]))
    mrcnn_probs.append(KL.TimeDistributed(KL.Activation("softmax"),
                                     name="mrcnn_class1")(mrcnn_class_logits[1]))
    mrcnn_probs.append(KL.TimeDistributed(KL.Activation("softmax"),
                                     name="mrcnn_class2")(mrcnn_class_logits[2]))
    mrcnn_probs.append(KL.TimeDistributed(KL.Activation("softmax"),
    ...
    relational_graph_probs=KL.TimeDistributed(KL.Activation("softmax"),name="mrcnn_class5")                                        
    for i in range(len(relational_graph_input)):
        z=relational_graph_probs(relational_graph_output['logits'][i])
        relational_graph_output['probs'].append(relational_graph_reshape(z))
        
    
    rel_logits=KL.Lambda(lambda x: K.stack(x,axis=4), name="rel_logits_stack1")(relational_graph_output['logits'])    
    mrcnn_class_logits.append(rel_logits) 
    
    rel_probs=KL.Lambda(lambda x: K.stack(x,axis=4), name="rel_probs_stack2")(relational_graph_output['probs'])    
    mrcnn_probs.append(rel_probs) 
    
    
    print('checkup',mrcnn_class_logits[5].shape)
    print('checkup1',mrcnn_probs[5].shape)
    # BBox head
    ...
    print('checkup2',mrcnn_poses.shape)
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    """
    print('shapwssssssss',target_class_ids.shape)
...
def mrcnn_class_rel_loss_graph(target_class_ids, pred_class_logits,
                           active_class_ids):
    """Loss for the classifier head of Mask RCNN.

    target_class_ids: [batch, num_rois, num_rois,4]. Integer class IDs. Uses zero
        padding to fill in the array.
    pred_class_logits: [batch, num_rois,num_rois,num_classes,4]
    active_class_ids: [batch, num_classes]. Has a value of 1 for
        classes that are in the dataset of the image, and 0
        for classes that are not in the dataset.
    """
    
    #remove adversary relationships
    
    #transform target_class_ids from [batch, num_rois, num_rois,4] to [batch, num_rois*num_rois*4]
    
    _,nber_boxes,nber_boxes,channels=target_class_ids.get_shape()
    batch_size=1
    print(batch_size,nber_boxes,nber_boxes,channels)
    channels=4
    obj_feature_pairs=[]
    list_of_valid_obj=[]
    for k in range(batch_size):
        list_of_valid_obj.append(tf.reshape(tf.where(tf.count_nonzero(tf.count_nonzero(target_class_ids[k],2),0)),[-1]))
        print(list_of_valid_obj[k].shape)
        if(list_of_valid_obj[k].shape[0]==0):
            #All are adversary relationships/No backpropagation
            return K.constant(value=0.0,shape=[])
        obj_feature_pairs.append(tf.reshape(tf.concat(values=[tf.gather(tf.gather(target_class_ids[k,:,:,i],list_of_valid_obj[k],axis=0),list_of_valid_obj[k],axis=1) for i in range(channels)],axis=1),[-1]))
    target_class_ids=tf.stack(obj_feature_pairs,axis=0)
    print('target_class_ids',target_class_ids.shape)
    print('hello')
    #transform pred_class_logits from [batch, num_rois, num_rois,num_classes,4] to [batch, num_rois*num_rois*4,num_classes]
    print('=pred_class_logits',pred_class_logits.shape)
    _,nber_boxes,nber_boxes,num_classes,channels=pred_class_logits.get_shape()
    channels=4
    print('=pred_class_logits',pred_class_logits.shape)
    del obj_feature_pairs[:]
    obj_feature_pairs=[]
    print('batch_size',batch_size)
    for k in range(batch_size):
        print('batch_size',batch_size)
        obj_feature_pairs.append(tf.reshape(tf.concat(values=[tf.gather(tf.gather(pred_class_logits[k,:,:,:,i],list_of_valid_obj[k],axis=0),list_of_valid_obj[k],axis=1) for i in range(channels)],axis=1),[-1,num_classes]))
    pred_class_logits=tf.stack(obj_feature_pairs,axis=0)
    print('pred_class_logits',pred_class_logits.shape)
    ...
    # classes of the image.
    loss = loss * pred_active
    print('bonsoir',pred_active.shape)
    # Computer loss mean. Use only predictions that contribute
    # to the loss to get a correct mean.
    loss = tf.reduce_sum(loss) / tf.reduce_sum(pred_active)
    print('bonsoirhoo')
...
def mrcnn_poses_loss_graph(target_poses, target_class_ids, pred_poses):
    """Loss for Mask R-CNN bounding box refinement.

    target_poses: [batch, num_rois, (tetax,tetay,tetay,x,y,z)]
    target_class_ids: [batch, num_rois]. Integer class IDs.
    pred_poses: [batch, num_rois, num_classes(tetax,tetay,tetay,x,y,z)]
    """
    # Reshape to merge batch and roi dimensions for simplicity.
    target_class_ids = K.reshape(target_class_ids, (-1,))
    target_poses = K.reshape(target_poses, (-1, 6))
    pred_poses = K.reshape(pred_poses, (-1, K.int_shape(pred_poses)[2], 6))
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    relations:[instance_count,instance_count,4]
    poses:[instance_count,6]
    ...
    class_rel_ids=class_rel_ids[_idx,:,:][:,_idx,:]
    poses=poses[_idx]
    list_class_ids=[class_cat_ids,class_col_ids,class_sha_ids,class_mat_ids,class_opn_ids,class_rel_ids]
    ...
    source_class_cat_ids = dataset.source_class_ids[0][dataset.image_info[image_id]["source"]]
    active_class_cat_ids[source_class_cat_ids] = 1

    active_class_col_ids = np.zeros([dataset.num_classes[1]], dtype=np.int32)
    source_class_col_ids = dataset.source_class_ids[1][dataset.image_info[image_id]["source"]]
    active_class_col_ids[source_class_col_ids] = 1
    
    active_class_sha_ids = np.zeros([dataset.num_classes[2]], dtype=np.int32)
    source_class_sha_ids = dataset.source_class_ids[2][dataset.image_info[image_id]["source"]]
    active_class_sha_ids[source_class_sha_ids] = 1
    
    active_class_mat_ids = np.zeros([dataset.num_classes[3]], dtype=np.int32)
    source_class_mat_ids = dataset.source_class_ids[3][dataset.image_info[image_id]["source"]]
    active_class_mat_ids[source_class_mat_ids] = 1
    
    active_class_opn_ids = np.zeros([dataset.num_classes[4]], dtype=np.int32)
    source_class_opn_ids = dataset.source_class_ids[4][dataset.image_info[image_id]["source"]]
    active_class_opn_ids[source_class_opn_ids] = 1
    
    active_class_rel_ids = np.zeros([dataset.num_classes[5]], dtype=np.int32)
    source_class_rel_ids = dataset.source_class_ids[5][dataset.image_info[image_id]["source"]]
    active_class_rel_ids[source_class_rel_ids] = 1
    ...
    # Image meta data
    active_class_ids=[active_class_cat_ids,active_class_col_ids,active_class_sha_ids,active_class_mat_ids,active_class_opn_ids,active_class_rel_ids]
    image_meta = compose_image_meta(image_id, shape, window, active_class_ids)

    return image, image_meta, list_class_ids, bbox, mask,poses


def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, gt_poses, config):
    ...
    gt_class_ids: [NUM_FEATURES,instance count] Integer class IDs/output feature
    relations:[instance count,instance count,4]
    gt_boxes: [instance count, (y1, x1, y2, x2)]
    gt_masks: [height, width, instance count] Grund truth masks. Can be full
              size or mini-masks.
    gt_poses: [instance count, (tetax,tetay,tetaz,x,y,z)]
    ...
    poses:  [TRAIN_ROIS_PER_IMAGE, NUM_CLASSES, (tetax,tetay,tetaz,x,y,z)]
    """
    assert rpn_rois.shape[0] > 0
    for i in range(config.NUM_FEATURES-1):
        assert gt_class_ids[i].dtype == np.int32, "Expected int but got {}".format(
            gt_class_ids[i].dtype)
    assert gt_boxes.dtype == np.int32, "Expected int but got {}".format(
        gt_boxes.dtype)
    assert gt_masks.dtype == np.bool_, "Expected bool but got {}".format(
        gt_masks.dtype)
    assert gt_poses.dtype == np.float32, "Expected float32 but got {}".format(
        gt_poses.dtype)
    ...
    instance_ids = np.where(gt_class_ids[0] > 0)[0]
    assert instance_ids.shape[0] > 0, "Image must contain instances."
    gt_class_ids[0] = gt_class_ids[0][instance_ids]
    gt_class_ids[1] = gt_class_ids[1][instance_ids]
    gt_class_ids[2] = gt_class_ids[2][instance_ids]
    gt_class_ids[3] = gt_class_ids[3][instance_ids]
    gt_class_ids[4] = gt_class_ids[4][instance_ids]
    gt_class_ids[5] = gt_class_ids[5][instance_ids,:,:][:,instance_ids,:]
    gt_boxes = gt_boxes[instance_ids]
    gt_masks = gt_masks[:, :, instance_ids]
    gt_poses = gt_poses[instance_ids]
    ...
    rpn_roi_gt_poses = gt_poses[rpn_roi_iou_argmax]
    rpn_roi_gt_class_ids=[]
    rpn_roi_gt_class_ids.append(gt_class_ids[0][rpn_roi_iou_argmax])
    rpn_roi_gt_class_ids.append(gt_class_ids[1][rpn_roi_iou_argmax])
    rpn_roi_gt_class_ids.append(gt_class_ids[2][rpn_roi_iou_argmax])
    rpn_roi_gt_class_ids.append(gt_class_ids[3][rpn_roi_iou_argmax])
    rpn_roi_gt_class_ids.append(gt_class_ids[4][rpn_roi_iou_argmax])
    rpn_roi_gt_class_ids.append(gt_class_ids[5][rpn_roi_iou_argmax,:,:][:,rpn_roi_iou_argmax,:])
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    rpn_roi_gt_poses[keep_bg_ids, :] = 0
    rpn_roi_gt_class_ids[0][keep_bg_ids] = 0
    rpn_roi_gt_class_ids[1][keep_bg_ids] = 0
    ...
    roi_gt_poses = rpn_roi_gt_poses[keep]
    roi_gt_class_ids=[]
    roi_gt_class_ids.append(rpn_roi_gt_class_ids[0][keep])
    roi_gt_class_ids.append(rpn_roi_gt_class_ids[1][keep])
    ...
    bboxes = np.zeros((config.TRAIN_ROIS_PER_IMAGE,
                       config.NUM_CLASSES[0], 4), dtype=np.float32)
    pos_ids = np.where(roi_gt_class_ids[0] > 0)[0]
    ...
    masks = np.zeros((config.TRAIN_ROIS_PER_IMAGE, config.MASK_SHAPE[0], config.MASK_SHAPE[1], config.NUM_CLASSES[0]),
                     dtype=np.float32)
    for i in pos_ids:
        class_id = roi_gt_class_ids[0][i]
        ...
        try:
            ...
            if random_rois:
                ...
                if detection_targets:
                    rois, mrcnn_class_ids, mrcnn_bbox, mrcnn_mask,mrcnn_poses =\
                        build_detection_targets(
                            rpn_rois, gt_class_ids, gt_boxes, gt_masks,gt_poses, config)
                    [mrcnn_class_cat_ids,mrcnn_class_col_ids,mrcnn_class_sha_ids,mrcnn_class_mat_ids,mrcnn_class_opn_ids,mrcnn_class_rel_ids]=mrcnn_class_ids
            ...
            if b == 0:
                ...
                batch_gt_class_rel_ids = np.zeros(
                    (batch_size,config.MAX_GT_INSTANCES,config.MAX_GT_INSTANCES,4), dtype=np.int32)
                ...
                if random_rois:
                    ...
                    if detection_targets:
                        ...
                        batch_mrcnn_class_rel_ids = np.zeros(
                            (batch_size,)+mrcnn_class_cat_ids.shape+mrcnn_class_cat_ids.shape+(4,), dtype=mrcnn_class_rel_ids.dtype)
    
                        batch_mrcnn_bbox = np.zeros(
                            (batch_size,) + mrcnn_bbox.shape, dtype=mrcnn_bbox.dtype)
                        batch_mrcnn_mask = np.zeros(
                            (batch_size,) + mrcnn_mask.shape, dtype=mrcnn_mask.dtype)
                        batch_mrcnn_poses = np.zeros(
                            (batch_size,) + mrcnn_poses.shape, dtype=mrcnn_poses.dtype)
            ...
            batch_gt_class_rel_ids[b,:gt_class_rel_ids.shape[0],:gt_class_rel_ids.shape[1],:gt_class_rel_ids.shape[2]] = gt_class_rel_ids
            ...
            if random_rois:
                ...
                if detection_targets:
                    ...
                    batch_mrcnn_class_rel_ids[b,:mrcnn_class_rel_ids.shape[0],:mrcnn_class_rel_ids.shape[1],:mrcnn_class_rel_ids.shape[2]] = mrcnn_class_rel_ids
                    batch_mrcnn_bbox[b] = mrcnn_bbox
                    batch_mrcnn_mask[b] = mrcnn_mask
                    batch_mrcnn_poses[b] = mrcnn_poses
            b += 1
            ...
            if b >= batch_size:
                ...
                if random_rois:
                    ...
                    if detection_targets:
                        ...
                        batch_mrcnn_class_rel_ids = np.expand_dims(batch_mrcnn_class_rel_ids, -1)
                        outputs.extend(
                            ...
                            batch_mrcnn_class_rel_ids,batch_mrcnn_bbox, batch_mrcnn_mask,batch_mrcnn_poses])
...
class RobotVQA():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            input_gt_class_rel_ids = KL.Input(
                shape=[None,None,4], name="input_gt_class_rel_ids", dtype=tf.int32)
        ...
        if mode == "training":
            ...
            active_class_ids=output[3:]
            [active_class_cat_ids,active_class_col_ids,active_class_sha_ids,active_class_mat_ids,active_class_opn_ids,active_class_rel_ids]=active_class_ids
            ...
            target_class_ids=DetectionTargetLayer_output[1:7]
            target_bbox=DetectionTargetLayer_output[7]
            target_mask=DetectionTargetLayer_output[8]
            target_poses=DetectionTargetLayer_output[9]
            #target_class_ids is a 2-3D tensor
            target_class_cat_ids=target_class_ids[0]
            target_class_col_ids=target_class_ids[1]
            target_class_sha_ids=target_class_ids[2]
            target_class_mat_ids=target_class_ids[3]
            target_class_opn_ids=target_class_ids[4]
            target_class_rel_ids=target_class_ids[5]
            ...
            print('shapuuuu',class_cat_loss.shape)
            ...
            print('shapuuuu',class_col_loss.shape)
            ...
            print('shapuuuu',class_sha_loss.shape)
            ...
            poses_loss = KL.Lambda(lambda x: mrcnn_poses_loss_graph(*x), name="mrcnn_poses_loss")(
                [target_poses, target_class_cat_ids, robotvqa_poses])
            print('Here, we go1')
            ...
            print('Here, we go')
            # Model
            print('\n\n\n',robotvqa_class_cat._keras_history,'\n\n\n')  
            inputs = [input_image, input_image_meta,
                      input_rpn_match, input_rpn_bbox,
                       input_gt_class_cat_ids,input_gt_class_col_ids,input_gt_class_sha_ids, input_gt_class_mat_ids,input_gt_class_opn_ids,input_gt_class_rel_ids,
                       input_gt_boxes, 
                       input_gt_masks,input_gt_poses
                     ]
            if not config.USE_RPN_ROIS:
            ...
            #relations is [batch, num_detections, num_detections, 9]  in image coordinates
    ...
    def get_relational_graph_input(self,objects,questions):
        #input for relational network
        ...
        relational_graph_input=KL.Lambda(lambda x: K.stack(x,axis=1), name="rel_stack0")(obj_feature_pairs)  
        print('\n\n\n',relational_graph_input._keras_history,'\n\n\n')  
    ...
    def parse_image_meta_graph(self,meta):
        ...
        [N1,N2,N3,N4,N5,N6,N7]=self.config.NUM_CLASSES
        ...
        active_class_ids.append(meta[:, 8+N1+N2+N3+N4+N5:8+N1+N2+N3+N4+N5+N6])
    ...
    def compile(self, learning_rate, momentum):
        ...
        loss_names = ["rpn_class_loss", "rpn_bbox_loss",
                      "mrcnn_class_loss","mrcnn_class_col_loss",
                      "mrcnn_class_sha_loss","mrcnn_class_mat_loss","mrcnn_class_opn_loss","mrcnn_class_rel_loss",
                       "mrcnn_bbox_loss", "mrcnn_mask_loss",'mrcnn_poses_loss']
            ...
            image_meta = compose_image_meta(
                ...
                [np.zeros([self.config.NUM_CLASSES[0]], dtype=np.int32),
                 ...
                 np.zeros([self.config.NUM_CLASSES[4]], dtype=np.int32),
                  np.zeros([self.config.NUM_CLASSES[5]], dtype=np.int32)
                ])
            # Append
    ...
    def unmold_detections(self, detections,relations, mrcnn_mask, image_shape, window):
        """Reformats the detections of one image from the format of the neural
        network output to a format suitable for use in the rest of the
        application.

        detections: [N, (y1, x1, y2, x2, class_ids, scores,tetax,tetay,tetaz,x,y,z)]
        relations:[N,N,8] relations+scores
        ...
        poses = detections[:N, detections.shape[1]-6:detections.shape[1]]
        class_ids=[]
        scores=[]
        ...
        class_ids.append(relations[:,:,:4])
        scores.append(relations[:,:,4:])
        ...
        if exclude_ix.shape[0] > 0:
            ...
            for i in range(self.config.NUM_FEATURES-2):
                class_ids[i] = np.delete(class_ids[i], exclude_ix, axis=0)
                scores[i]= np.delete(scores[i], exclude_ix, axis=0)
            class_ids[5] = np.delete(class_ids[5], exclude_ix, axis=0)
            class_ids[5] = np.delete(class_ids[5], exclude_ix, axis=1)
            scores[5]= np.delete(scores[5], exclude_ix, axis=0)
            scores[5]= np.delete(scores[5], exclude_ix, axis=1)
            masks = np.delete(masks, exclude_ix, axis=0)
            N = class_ids[0].shape[0]
    ...
    def detect(self, images, verbose=0):
        ...
        poses: [N, (tetax,tetay,tetaz,x,y,z)] detection objects' poses
        ...
        detections,relations, mrcnn_class_cat, mrcnn_class_col, mrcnn_class_sha, mrcnn_class_mat, mrcnn_class_opn, mrcnn_class_rel, mrcnn_bbox, mrcnn_mask, \
            rois, rpn_class, rpn_bbox, mrcnn_poses =\
            self.keras_model.predict([molded_images, image_metas], verbose=0)
        # Process detections
        results = []
        for i, image in enumerate(images):
            final_rois, \
            final_class_cat_ids, final_class_col_ids, final_class_sha_ids, final_class_mat_ids, final_class_opn_ids,final_class_rel_ids, \
            final_cat_scores,final_col_scores,final_sha_scores,final_mat_scores,final_opn_scores,final_rel_scores, \
            final_masks,final_poses =\
                self.unmold_detections(detections[i],relations[i],mrcnn_mask[i],
                                       image.shape, windows[i])
            results.append({
                "rois": final_rois,
                "class_cat_ids": final_class_cat_ids,"class_col_ids": final_class_col_ids,"class_sha_ids": final_class_sha_ids,
                "class_mat_ids": final_class_mat_ids,"class_opn_ids": final_class_opn_ids, "class_rel_ids":final_class_rel_ids,               
                "scores_cat": final_cat_scores,"scores_col": final_col_scores,"scores_sha": final_sha_scores,"scores_mat": final_mat_scores,
                "scores_opn": final_opn_scores,'scores_rel':final_rel_scores,
                "masks": final_masks,"poses":final_poses
            })
        return results
...
def parse_image_meta_graph(meta,config):
    ...
    image_shape = meta[:, 1:4]
    window = meta[:, 4:8]   # (y1, x1, y2, x2) window of image in in pixels
    [N1,N2,N3,N4,N5,N6,N7]=config.NUM_CLASSES
    active_class_ids=[meta[:, 8:8+N1]]
    active_class_ids.append(meta[:, 8+N1:8+N1+N2])
    active_class_ids.append(meta[:, 8+N1+N2:8+N1+N2+N3])
    active_class_ids.append(meta[:, 8+N1+N2+N3:8+N1+N2+N3+N4])
